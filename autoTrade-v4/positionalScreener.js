...
function _psFutTokenFor(name) {
    ...
    if (_PS_MCX_NAMES.indexOf(name) !== -1) {
        ...
        var mcxM = COMMODITIES_FUTURE_INSTRUMENT_LIST.find(function (r) { return r.name === exchName; });
        return mcxM ? mcxM.instrument_token : null;
    }
    if (typeof FUTURE_INTRUMENT_LIST === 'undefined') return null;
    var m = FUTURE_INTRUMENT_LIST.find(function (r) { return r.name === exchName; });
    return m ? m.instrument_token : null;
}
...
function _psComputeSetup(candles, futCandles, niftyPctChg20, futToken) {
    ...
    if (futCandles && futCandles.length >= 6) {
    ...
    try {
        ...
        for (var i = 0; i < universe.length; i++) {
            ...
            try {
                ...
                var futToken = _psFutTokenFor(name);
                ...
                var setup = _psComputeSetup(candles, futCandles, niftyPctChg20, futToken);
