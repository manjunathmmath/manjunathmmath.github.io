...
function showWebSocketPopup() {
    ...
    jQ('#qw-ws-search').on('input', function () {
        ...
        var names = Object.keys(INSTRUMENT_TOKENS).filter(function (n) {
            return n.indexOf(q) !== -1 && !_QW_SUBSCRIBED[INSTRUMENT_TOKENS[n]];
        }).slice(0, 8);
        if (!names.length) { jQ('#qw-ws-suggest').hide().empty(); return; }
        var h = names.map(function (n) {
            return '<div class="qw-ws-sugg-row" data-name="' + n + '" style="padding:4px 8px;cursor:pointer;">' + n + '</div>';
        }).join('');
